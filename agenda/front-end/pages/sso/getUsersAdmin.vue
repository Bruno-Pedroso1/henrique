...
<template>
  <v-container>
    <v-row>
      <v-col>
        <h1 class="d-flex align-center flex-column">Administradores</h1>
      </v-col>
    </v-row>
...
export default {
  ...
  methods: {
    ...
    async getAdminUsers() {
      try {
        const response = await this.$api.get("/api/users-admin");
        const filtro = response.filter((a) => a.permission == null);
        this.items = filtro;
